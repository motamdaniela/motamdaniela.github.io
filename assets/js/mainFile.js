...
opts.forEach((opt) => {
    opt.addEventListener('mouseenter', function(){
        ...
        opts.forEach(option => {
            ...
            // else if(option.id == opt.id){
                // document.querySelector(`.${option.id}`).style = 
                // `background-image: url('../img/circle.svg');
                // background-repeat: none;
                // background-size: 120%;`;

                // document.querySelector(`.${opt.id}`).style.background = '';
                // document.querySelector(`.${opt.id}`).style.backgroundSize = '90px';
            // }
        })
